...
def create_model(vocab_size, num_labels):
  model = tf.keras.Sequential([
      ...
      layers.Conv1D(64, 5, padding="valid", activation=args.activation, strides=2),
...
history = int_model.fit(int_train_ds, validation_data=int_val_ds, epochs=5, callbacks=[vertex_hpt_callback, tensorboard_callback])
...
logging.info("Model loss: {:2.2%}".format(int_loss))
...
logging.info("Loss: {:2.2%}".format(loss))
